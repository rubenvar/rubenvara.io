...
<header>
    <p>Dicen que no sabes lo que sabes hasta que intentas enseñarlo, así que en eso estamos:</p>
    <p>Escribo sobre desarrollo web en español pero me falta vocabulario, porque todo lo que sé lo he aprendido en inglés.</p>
    <p>
    ...
    {#if dev && categories}
        ...
        <button onclick={handleStatusClick}>
            See {#if filterStatus === 'all'}published only{:else if filterStatus === 'published'}draft only{:else}all{/if}
        </button>
    {/if}
</header>
