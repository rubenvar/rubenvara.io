...
export async function getAllPosts(options?: AllPostsOptions) {
    ...
    return (
        allPosts
            ...
            // sort by date here
            .sort((a, b) => (new Date(b.updated || b.date) < new Date(a.updated || a.date) ? -1 : 1))
...
export async function getAllCategories(): Promise<Category[]> {
    ...
    allCategories.forEach((obj) => {
        categoriesCounted[obj.category] = (categoriesCounted[obj.category] || 0) + 1;
    });
...
export function getCategoryCount(category: string) {
    ...
    if (!dev) {
        // in prod, only 'published' posts
        allPostsInCategory = allPostsInCategory.filter((post) => post.status === 'published');
    }
...
export async function getSinglePost(category: string, slug: string) {
    ...
    if (!postResolver) {
        error(404, `no post "/${category}/${slug}" found (in [slug]/+page.server.ts)`);
    }

    const { metadata, default: content } = await postResolver();

    // metadata of posts in a series (if post is in a series)
    const postsInSeries = metadata.series?.index ? getPostsInSeries(metadata.series.name) : undefined;
