...
{#if dev && (link || words)}
    <div>
        {#if link}
            <ul>
                <li class:has-inner={linkedFrom.length >= 1} class:dang={linkedFrom.length < 1} class:succ={linkedFrom.length >= 1}>
                ...
                <li class:has-inner={link.internalTotal >= 1} class:warn={link.internalTotal < 1} class:succ={link.internalTotal >= 1}>
                ...
                <li class:has-inner={link.externalTotal >= 1} class:dang={link.externalTotal < 1} class:succ={link.externalTotal >= 1}>
        ...
        {#if words}
            <ul>
                <li>
                    Word count: {words.wordCount}. {#if words?.wordCount > 0}Higher than {wordsPercent}% of all posts{/if}
                </li>
                <li>
                    Read time: {Math.ceil(words.minutes)} minutes ({Math.floor(words.minutes * 100) / 100})
                </li>
            </ul>
        {/if}
    </div>
