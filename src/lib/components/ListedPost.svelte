...
<article>
    <a href={resolve(`/${post.category}/${post.slug}`)}>
        <div>
            <h2>
            ...
            {#if dev && post.status === 'draft' && !post.description}
                <p class="description">
                    Draft: Lorem ipsum dolor sit amet consectetur adipisicing elit. Aspernatur facilis, doloremque repellendus culpa, illum enim aliquid, maxime
                    fugit amet
                </p>
            {/if}
            {#if index}
            ...
            {/if}
            <ListedPostMeta date={post.date} updated={post.updated} category={post.category} {inCategoryPage} />
        </div>
    </a>
</article>
