...
Esto pasa porque React Query expone diversa meta-información sobre cada solicitud, e `isFetching` es parte de ella. Esta `flag` será siempre `true` cuando se esté ejecutando una solicitud. Esto es **muy útil** si quieres mostrar un indicador de progreso, pero bastante inútil si no vas a hacerlo.
...
¿Qué pasa si quisiéramos *reaccionar* a los errores? ¿O si empezamos a usar la *flag* `isLoading`? Tendríamos que mantener la lista `notifyOnChangeProps` sincronizada con cualquier campo que estemos usando en los componentes donde se use este *hook*.
